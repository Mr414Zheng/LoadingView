...
/**
 * Created by zhenghuaizhi on 2019/3/14
 * 内存占用比LoadingView高
 */
public class LoadingSurfaceView extends SurfaceView {
     ...
     * @param attrs
     */
    public LoadingSurfaceView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LoadingSurfaceView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void start() {
        isRunning = true;
        if (drawThread == null) {
            mThreadPool = new ThreadPoolExecutor(1, 2, 500, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>());
            drawThread = new DrawThread();
            mThreadPool.execute(drawThread);
            startAnim();
        }
    }
    ...
    private void intiAttrs(Context context, @Nullable AttributeSet attrs) {
        // 加载自定义属性集合
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.LoadingSurfaceView);
        // 实现动画效果，lines值不得少于3；
        mLines = typedArray.getInteger(R.styleable.LoadingView_lines, 3);
        mLines = mLines < 3 ? 3 : mLines;
        // 如果设置的值是Xdp | Xsp，则返回X * density; 如果设置的是Xpx，则返回X；
        mBlockWidthHalf = typedArray.getDimension(R.styleable.LoadingSurfaceView_layout_blockWidth_half1, 30f);
        mBlockMargin = typedArray.getDimension(R.styleable.LoadingSurfaceView_layout_blockMargin1, 10f);
        mStartPosition = typedArray.getInteger(R.styleable.LoadingSurfaceView_layout_startPosition1, 0);
        ...
        mMoveBlockAngle = typedArray.getFloat(R.styleable.LoadingSurfaceView_moveBlock_angle1, 10f);
        mFixBlockAngle = typedArray.getFloat(R.styleable.LoadingSurfaceView_fixBlock_angle1, 30f);
        mBlockColor = typedArray.getColor(R.styleable.LoadingSurfaceView_blockColor1, Color.MAGENTA);
        mClockwise = typedArray.getBoolean(R.styleable.LoadingSurfaceView_isClockwise1, true);
        mDuration = typedArray.getInteger(R.styleable.LoadingSurfaceView_duration1, 250);
        mInterpolatorId = typedArray.getResourceId(R.styleable.LoadingSurfaceView_interpolator1, android.R.anim.bounce_interpolator);
